...
<body>
	...
	<a href="/ui/projectform" class="createLink">Create project</a>
	<table id="table" class="" width="100%"></table>
	<script>
	        ...
	        axios.get('/projects').then(response => {
	        	...
	        	for (var i = 0; i < response.data.length;i++){
	        		...
	        		for (var j = 0; j < response.data[i].users.length; j++){
	        			projUsers.push(response.data[i].users[j].username);
	        		}
	        		tableData.push([response.data[i].id,response.data[i].name, response.data[i].desc, response.data[i].manager.username, projUsers.join(', '),response.data[i].id]);
	        	}
	        	$(document).ready(function() {
	        	    $('#table').DataTable( {
	        	        data: tableData,
	        	        columns: [
	        	        	{ title: "Id", render: function(data, type, row, meta){ return '<a href="/ui/projectform/'+data+'">'+data+'</a>' }},
	        	            { title: "Project name" },
	        	            { title: "Description" },
	        	            { title: "Manager" },
	        	            { title: "Users" },
	        	            { title: "", render: function(data, type, row, meta){ return (getRoles().includes('admin') || getRoles().includes('manager')) ? '<a href="javascript:deleteObject('+data+',\'projects\')">Delete</a>' : '' } }
	        	        ]
	        	    } );
	        	} );
	            });
