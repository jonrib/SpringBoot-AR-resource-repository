...
<body>
	...
	<a href="/ui/groupform" class="createLink">Create group</a>
	    ...
	    methods: {
	        ...
	        axios.get('/groups').then(response => {
	        	...
	        	for (var i = 0; i < response.data.length;i++){
	        		tableData.push([response.data[i].id,response.data[i].groupName, response.data[i].manager.username,response.data[i].id]);
	        	}
	        	$(document).ready(function() {
	        	    $('#table').DataTable( {
	        	        data: tableData,
	        	        columns: [
	        	        	{ title: "Id", render: function(data, type, row, meta){ return '<a href="/ui/groupform/'+data+'">'+data+'</a>' }},
	        	            { title: "Group name" },
	        	            { title: "Manager" },
	        	            { title: "", render: function(data, type, row, meta){ return '<a href="javascript:deleteObject('+data+',\'groups\')">Delete</a>' } }
