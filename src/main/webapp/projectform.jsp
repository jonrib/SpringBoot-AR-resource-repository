...
<body>
	...
	<div class="container page-content">
		<form class="form-signin">
			<h2 class="form-signin-heading">Project</h2>
	...
	$(document).ready(function(){
		...
		if (id != 'projectform'){
			$.ajax({
				...
				$('#project-name').val(jsonData.name);
				$('#project-desc').val(jsonData.desc);
				var users = [];
				for (var i = 0; i < jsonData.users.length; i++){
					users.push(jsonData.users[i].username);
				}
	...
	function doSubmit(){
		var hasErr = false;
		$('#project-name').removeClass('has-error');
		$('#project-desc').removeClass('has-error');
		...
		proj.data = {};
		proj.users = {};
		proj.data.name = $('#project-name').val();
		proj.data.desc = $('#project-desc').val();
		proj.users.manager = $('#managerSelect').val();
		proj.users.users = $('#userSelect').val();
		
		$.ajax({
			url: "/projects"+(window.location.href.substring(window.location.href.lastIndexOf('/')+1) == 'projectform' ? '' : '/'+window.location.href.substring(window.location.href.lastIndexOf('/')+1)),
			type: window.location.href.substring(window.location.href.lastIndexOf('/')+1) == 'projectform' ? "POST" : "PUT",
